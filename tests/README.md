...
**Что тестируется:**
- OCR штампа: парсинг даты/времени/индекса из Timestamp Camera (parseStampText)
- OCR подписи: парсинг имени и типа события (parseCaptionText)
- Все русские месяцы, варианты текста, граничные случаи
- МСК → UTC конвертация
- Дефекты OCR (точки вместо двоеточий, регистр, многострочность)

**Результат:** Все 21 тест должны быть GREEN ✅
...
- Вставляет 5 событий pending, покрывающих разные сценарии:
  ...
  - Wrong location флаг → ожидается needs_review (fraud)
  - Опечатка OCR (fuzzy match) → ожидается matched
