...
### 10.2 Деплой
- [ ] 14. Закоммитить: `git commit -m "remove OCR from webhook"`
- [ ] 15. Задеплоить: `vercel --prod`

### 10.3 Проверка
- [ ] 16. Отправить "Иван начало смены" в группу → в `logs` появился `"Photo received"`
- [ ] 17. В `events` появилось событие со статусом `pending`
- [ ] 18. В Storage лежит фото `photos/{chatId}/{messageId}.jpg`
- [ ] 19. Через 5 минут ai-worker обработал → статус `done` или `needs_review`
- [ ] 20. Проверка face recognition: отправить фото не того человека → флаг `face_mismatch`
- [ ] 21. Проверка fuzzy match: отправить "Иван Пертов" (опечатка) → сотрудник найден
- [ ] 22. Дашборд показывает событие и фото корректно
