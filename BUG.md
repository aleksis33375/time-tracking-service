...
- **Приоритет:** 🔴 КРИТИЧЕСКИЙ
- **Статус:** Не исправлено
- **Описание:** Upload может упасть, но функция вернёт путь → сохранит несуществующий URL в БД.
- **Решение:**
```js
...
- **Приоритет:** 🔴 КРИТИЧЕСКИЙ
- **Статус:** Не исправлено
- **Описание:** Пустой экспорт без предупреждения → путанница пользователя.
- **Решение:**
```js
...
- **Приоритет:** 🔴 КРИТИЧЕСКИЙ
- **Статус:** Не исправлено
- **Описание:** Нет проверки формата, дубликатов, валидности данных → импорт некорректных данных в БД.
- **Решение:** Функция `validateEmployeeRow()` с полной валидацией каждого поля перед импортом.
