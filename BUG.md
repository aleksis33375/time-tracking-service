...
- **Файл:** `api/webhook.js`
- **Приоритет:** 🟠
- **Статус:** ✅ Исправлено (деплой через git push — на усмотрение пользователя)
- **Описание:** В `handlePhoto` поле `photo_timestamp` записывалось из `msg.date` (момент доставки сообщения в Telegram), а не из EXIF-поля `DateTimeOriginal` фото. Если работник снимал в 7:55, но отправлял в 9:30 (плохая связь), часы рассчитывались некорректно — в `ai-worker/process_events.py` это влияет на смену и зарплату.
- **Решение:** В `webhook.js` добавлена функция `extractExifTimestamp()` через `sharp().metadata()` + `exif-reader`. EXIF DateTimeOriginal трактуется как МСК → конвертируется в UTC (−3 ч). Если EXIF нет или невалиден — fallback на Telegram-time. Источник времени логируется в Supabase (`time_source: exif | telegram`). В `package.json` добавлена зависимость `exif-reader ^2.0.0`.
- **Ограничение:** Telegram стирает EXIF при обычной отправке фото (сервер пережимает JPEG). Чтобы фикс реально работал, фото должны отправляться **«как файл»** (Android: «Прикрепить → Файл», iOS: «Документ»), а не как обычное фото. При отсутствии EXIF факт логируется в консоль (`EXIF: absent`).
